...
// [[Rcpp::export]]
NumericVector searchDescReturnHits(std::vector< std::string > desc, std::vector< std::string > searchTerms) { // types fixed
  NumericVector searchResults;
  ...
  for (int i = 0; i < n; ++i) {
    ...
    for (int j = 0; j < o; ++j) {
      ...
      } else if (containsString(desc[i], searchTerms[j])) { // if a searh term in contained in the DESC column of i^th row
        //++searchTermHits;
        // append search term or number indicating row to a list of hits
        // use push i onto stack/queue?
...
/*** R
searchDescReturnHits(myAnnot[,3], c("mitochon", "integrase","ribosom","gibberellin"))
*/
